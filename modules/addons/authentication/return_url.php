...
$biz_no = $_GET['biz_content'];

$success = strpos($biz_no,"true");

if ( !empty( $success ) ) {
	$type = 'success';
	$title = '认证成功';
	$biz_no = substr( $biz_no, 26 , 32);
	Capsule::table('mod_authentication')->where('biz_no', $biz_no)->update([
		'validation' 	=> 'yes',
...
<h2 class='sa-title'><?php echo $title?></h2>
<h3 class='sa-title'>请回到网页端刷新</h3>
<?php } else {?>

function setPrivateKey( systemURL, type ) {
	$.ajax({
		type: "GET",
		url: systemURL + "/modules/gateways/authentication/genRSA.php?type=" + type,//提交的URL
		dataType:"json",
		success: function (data) {
			...
			if (data.status == "success") {
			    ...
			    $('#authenticationconfig textarea').val(privateKey);
...
function getalipayKey( systemURL, appid, privateKey ) {
	$('#authenticationconfig input[type=text]').val(appid);
	$('#authenticationconfig textarea').val(privateKey);
}
